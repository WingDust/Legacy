...
// Napi::String gre(const Napi::CallbackInfo& info){
Napi::Value gre(const Napi::CallbackInfo& info){
    ...
    if (info[0].IsString()){
        ...
        ii();
        // struct frameData * f =&ff;
        // int re =  extract(p,f);
        // std::cout << re << "\n";
        // std::cout << f->data[0] << "\n";
        // std::cout << f->width << "\n";
        // std::cout << f->height << "\n";
        // std::cout << f->height*f->width << "\n"<< std::endl;
        ...
        for (size_t i = 0; i < 3; i++)
        {
            std::cout << "1" << "\n";
            if (i==0){
                // Napi::Buffer<uint8_t> b1 = Napi::Buffer<uint8_t>::New(env,f->data[0],640*f->height);
                // Napi::Buffer<uint8_t> b1 = Napi::Buffer<uint8_t>::New(env,b,2);
                // arr.Set(0u,b1);
                // arr.Set(0u,0);
            }
            if (i==1){
                // Napi::Buffer<uint8_t> b1 = Napi::Buffer<uint8_t>::New(env,f->data[1],(640*f->height/4));
                // arr.Set(1u,b1);
                // arr.Set(1u,1);
            }
            if (i==2){
                // Napi::Buffer<uint8_t> b1 = Napi::Buffer<uint8_t>::New(env,f->data[i],(640*f->height/4));
                // arr.Set(2u,b1);
                // arr.Set(2u,2);
            }
            // Napi::TypedArrayOf<uint8_t>::New(env,(f->width*f->height)+(f->width*f->height/4)+(f->width*f->height/4),)

        }
        // arr.Set(3u,Napi::Number::New(env,f->width));
        // arr.Set(4u,Napi::Number::New(env,f->height));
        

        std::string result = helloUser("wings");
        // return Napi::String::New(env,result);
        return arr;
    }
    // throw Napi::TypeError::New( env, "Expected a path string" );
    return env.Null();
}

Napi::Object Init(Napi::Env env ,Napi::Object exports){
    exports.Set(
        Napi::String::New(env,"gre"),
        Napi::Function::New(env,gre)
    );
